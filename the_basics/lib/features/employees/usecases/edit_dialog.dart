...
void showEditEmployeeDialog(
  ...
  final isEditingSelf = employee.id == userController.employee.value.id;
  final isAdmin = employee.role == 'admin';
  final hasKierownikTag = employee.tags.contains('Kierownik');
  ...
  final fields = [
    ...
    MultiSelectDialogField(
      ...
      onSelectionChanged: (List<String> selected) {
        // Jeśli to admin edytujący siebie i ma tag Kierownik
        if (isEditingSelf && isAdmin && hasKierownikTag) {
          // Wymuś zachowanie tagu Kierownik
          if (!selected.contains('Kierownik')) {
            selected.add('Kierownik');
            showCustomSnackbar(
              context,
              'Administrator musi zachować tag "Kierownik"',
            );
          }
        }
        selectedTags.assignAll(selected);
    ...
    DialogActionButton(
      ...
      onPressed: () {
        try {
          ...
          if (isEditingSelf && isAdmin && hasKierownikTag && !selectedTags.contains('Kierownik')) {
            showCustomSnackbar(context, 'Nie możesz usunąć sobie tagu "Kierownik"');
            return;
          }
